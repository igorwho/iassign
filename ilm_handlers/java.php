...
class java implements ilm_handle {
  ...
  public static function show_activity_in_ilm ($iassign_statement_activity_item, $student_answer, $enderecoPOST, $view_teacherfileversion) {
    ...
    //D if ($USER->id==3) echo "<br/> * java.php: show_activity_in_ilm(.): {context}.id=" . $context->id . ", iassign_statement_activity_item.id=" . $iassign_statement_activity_item->id . ", view_teacherfileversion=" . $view_teacherfileversion . "<br/>\n";
    ...
    if ($view_teacherfileversion) { // get the exercise in Moodle data (teacher file)
      //D if ($USER->id==3) echo " * java.php: show_activity_in_ilm(.): iassign_statement_activity_item.id=" . $iassign_statement_activity_item->id . ": carrega " . $CFG->dirroot . '/mod/iassign/files_functions.php' . "<br/>\n";
      $fileid = "";
      $fs = get_file_storage();
      ...
      //D if ($USER->id==3) echo " * java.php: show_activity_in_ilm(.): #answer_files=" . count($answer_files) . "<br/>\n"; //exit;
      if ($answer_files == NULL) { // Error: no {files} were found in this context with these 3 itemid
        //D print "Error: {iassign_statement}.id=" . $iassign_statement_activity_item->id . " associated file not found!<br/>\n";
        $one_files = NULL;
        $one_files_id = -1;
        $one_files_filename = "<empty>";
        }
      else {
    ...
    //leo TODO or to use: if ($extension == "html" || $extension == "ivph") // if iLM is HTML5 is inside a frame
  ...
  /// Presents the iLM and its content
  ...
  /// Copy or build a new version of iLM
  ...
  //x public static function delete_ilm($ilm_id) {
  //x global $DB, $CFG;
  //x $fs = get_file_storage();
  //x $contextsystem = context_system::instance();
  //x $iassign_ilm = $DB->get_record('iassign_ilm', array('id' => $ilm_id));
  //x $files_jar = explode(",", $iassign_ilm->file_jar);
  //x foreach ($files_jar as $file_jar) {
  //x   $file = $fs->get_file_by_id($file_jar);
  //x   if ($file)
  //x     $fs->delete_area_files($contextsystem->id, 'mod_iassign', 'ilm', $file->get_itemid());
  //x   }
  //x $DB->delete_records("iassign_ilm", array('id' => $ilm_id));
  //x $DB->delete_records("iassign_ilm_config", array('iassign_ilmid' => $ilm_id)); //MOOC 2016
  //x // log event --------------------------------------------------------------------------------------
  //x iassign_log::add_log('delete_iassign_ilm', 'name: ' . $iassign_ilm->name . ' ' . $iassign_ilm->version, 0, $iassign_ilm->id);
  //x // log event --------------------------------------------------------------------------------------
  //x return $iassign_ilm->parent;
  //x }
  ...
  /// Perform all the necessary steps to import a new iLM
  //  It creates the directory "./mod/iassign/ilm/<iLM name>/<version>/" and the inserts in {iassign_ilm}
