...
// 2025/04/04 20:35:39
// Esta versao funciona tendo removido toda a complicadao das funcoes 'process_iassign_statement($data)' e ''after_execute()'!
// Porem, usando essa forma automatica (deixando tudo com o Moodle), precisamos alterar
//   {iassign_statement}.file    <- {files}.itemid  old?
//   {iassign_statement}.filesid <- {files}.id
//   {{files}.itemid             <- iassign_statement}.id
...
echo "restore_iassign_stepslib.php!process_iassign(.): this->source_course_id=" . $this->source_course_id . " is the {course}.id of source course! destination course.id=" . $data->course . "<br/>"; //exit;
// this->courseid this->source_course_id
  ...
  // From the origin Moodle was used {iassign_statement}.* and from {iassign_ilm}: name as ilm_name, version, type, extension
  // @param object $data The data in object form
  ...
  protected function process_iassign_statement ($data) {
    ...
    if ($destination_ilmid != NULL)
      ...
      $error_msg  = "Potention problem: the backup file has one iAssign activity to which the iLM " . $data->ilm_name . " was not found.<br/>\n";
      $error_msg .= "Please, try to install " . $data->ilm_name . " from the URL " . $data->url;
      // With any of the bellow warning, the restoring process is interrupted!
      // throw new restore_step_exception($error_msg);
      // debugging('Error in restore_iassign_stepslib.php!process_iassign_statement(.): the iLM ' . $data->ilm_name . ' is missing!', DEBUG_DEVELOPER);
      }
    $oldid = $data->id; // from source {course}.id: old {iassign_statement}.id

// 2023/04/26: introduzido alteracoes dos campos $data->iassignid, $data->iassign_ilmid para tentar funcionar restauro em base distinta (outros iMA) : 
//             ver ~/projetos/iMA/lms/itarefa/novo/versoes/iassign/backup/moodle2/restore_iassign_stepslib_2023_04_26.php
// 2025/03/17: introduzido alteracoes dos campos $data->file, $data->filesid para tentar funcionar restauro em base distinta (outros iMA) : via get_mappingid(.)
//             ver ~/projetos/iMA/lms/itarefa/novo/versoes/iassign/backup/moodle2/restore_iassign_stepslib_2025_03_17.php
    ...
    $msgD1 = "restore_iassign_stepslib.php!process_iassign_statement(.): <br/>" .
             ...
             " - original {iassign_ilm}.name=" . $data->ilm_name . " (nome usado para buscar iMA semelhante) destination_ilmid=" . $destination_ilmid . "<br/>\n"; //DEBUG
    echo $msgD1; //DEBUG
    $msgD .= $msgD1; // append
...
// assign: restore_assign_stepslib.php: $this->set_mapping('submission', $oldid, $newitemid, false, null, $this->task->get_old_contextid());
// /var/www/html/saw_limpo/mod/assign/backup/moodle2/restore_assign_stepslib.php
...
// get_mappingid('files', ID) : use "backup_ids_temp" temporary table, to recover new ID from "itemname" 'files', by old given ID
// /var/www/html/saw_limpo/backup/moodle2/restore_subplugin.class.php :
// + protected function set_mapping($itemname, $oldid, $newid, $restorefiles = false, $filesctxid = null, $parentid = null)
//   * To send ids pairs to backup_ids_table and to store them into paths
...
// https://aichatonline.org/app/11c10b6b9eaefd8fab8a34688ccf2d29
// Get the old context ID and the old item ID from the backup
$oldcontextid = $this->task->get_old_contextid(); // $data->oldcontextid;  // The old context ID (from the backup)
$olditemid = $data->id; // is the olditemid = {iassign_statement}.id = {files}.itemid  // The old item ID (from the backup)
// Get the restore map to find the mapping of old item ID to new item ID
//$teste = $this->get_mapping('files', $data->id);
//$str_aux = "";
//foreach ($teste as $item) { $str_aux .= array_keys(get_object_vars($item)) . " ; "; }

// Warning: foreach() argument must be of type array|object, bool given in /var/www/html/saw_limpo/mod/iassign/backup/moodle2/restore_iassign_stepslib.php on line 204
// Warning: Undefined variable $newitemid in /var/www/html/saw_limpo/mod/iassign/backup/moodle2/restore_iassign_stepslib.php on line 226
// - original {iassign_statement}.id=6305 (old id) :: newitemid='' (usando restoremap[][]) 

/*
$restoremap = $this->get_restore_map(); //leo NAO erro, Exception - Call to undefined method restore_iassign_activity_structure_step::get_restore_map()
$newitemid = "vazio";
// Map the old item ID to the new item ID
if (isset($restoremap[$oldcontextid][$olditemid])) { $newitemid = $restoremap[$oldcontextid][$olditemid]; }
else { // Handle case where no mapping is found (shouldn't happen if restore is set up correctly)
  debugging('No mapping found for the old item ID: ' . $olditemid, DEBUG_DEVELOPER);
  // return;
  }
*/
// $data1 = new stdClass(); // backup::VAR_ACTIVITYID
//    
$str_aux = "";
    $aux_file1    = $this->get_mappingid('files', $data->file); // get the new ID corresponding to the old {files}.id=$data->file
    $aux_filesid1 = $this->get_mappingid('files', $data->filesid); //TODO: depois precisa processar {files}.itemid para este {iassign_statement}.id!
    ...
    $msgD1 = " - original {iassign_statement}.id=" . $data->id . " (old id) :: get_mappingid('files'," . $data->file    . ")=" . $aux_file1    . "<br/>\n" . //DEBUG
             " - original {iassign_statement}.id=" . $data->id . " (old id) :: get_mappingid('files'," . $data->filesid . ")=" . $aux_filesid1 . "<br/>\n" . //DEBUG
    ...
    $new_iassign_statement->file                  = $data->id; // old {iassign_statement}.id //2025 $files_id; // $this->get_mappingid('files', $data->file);
...
//$ss=0; for ($i=0; $i<10000; $i++) $ss = $ss+$i;
    ...
    try {
      ...
      // With any of the bellow warning, the restoring process is interrupted!
      // throw new restore_step_exception($error_msg);
      debugging('Error in restore_iassign_stepslib.php!process_iassign_statement(.): the iLM ' . $data->ilm_name . '!', DEBUG_DEVELOPER);
      }
...
echo "restore_iassign_stepslib.php!process_iassign_submission(.): this->source_course_id=" . $this->source_course_id . "; old {issign}.id=" . $oldid . "<br/>"; //exit;
...
echo "restore_iassign_stepslib.php!process_iassign_submission_comment(.): this->source_course_id=" . $this->source_course_id . "; old {issign}.id=" . $oldid . "<br/>"; //exit;
  ...
  protected function after_execute () {
    ...
    //x 
    $this->add_related_files('mod_iassign', 'activity', null); //2025
    $this->add_related_files('mod_iassign', 'intro', null);
...
$countF = 0; // count the total of iAssign activities file


// $this->source_course_id=" . $this->source_course_id . " is the {course}.id of source course!<br/>"; //exit;
...
echo "restore_iassign_stepslib.php!after_execute(): original {course}.id=" . $this->source_course_id . "; here courseid=" . $course_id . "<br/>"; //exit;
...
$countF += count($files); //D
          ...
          if ($choosen_file != null) { // there is {files} to associate
            if ($iassign_statement->filesid != $choosen_file->id) { // need to update 'iassign_statement.filesid'?
              ...
              $newentry = new stdClass(); $newentry->id = $iassign_statement->id; $newentry->filesid = $choosen_file->id;
...
//2023--- linha abaixo estava comentada, voltei ela!
            ...
            if ($choosen_file->itemid != $iassign_statement->id) {
              ...
              $newentry = new stdClass(); $newentry->id = $choosen_file->id; $newentry->itemid = $iassign_statement->id;
