...
//  @calledby ./mod/iassign/locallib.php : add_edit_iassign() : $mform = new mod_iassign_form(...)
//  @see      ./mod/iassign/locallib.php : function update_iassign($param) : that will register in {iassign_statement}
class mod_iassign_form extends moodleform {
  ...
  function definition () {
    ...
    foreach ($igeom as $item)
      ...
      //D echo "iassign_form.php!definition(): COURSE->iassignid=" . $COURSE->iassignid . "<br/>\n"; // NAO def ", id=" . $id . 
    ...
    // $COURSE->iassignid is define in 'locallib.php!add_edit_iassign()' with "$COURSE->iassignid = $iassignid;"
  ...
  //]]>
    ...
    //  Author activity: 
    //  Last modified by:
    $mform->addElement('header', 'data_activity', get_string('data_activity', 'iassign'));        // data_activity      ='Information activity'
    $mform->addElement('static', 'author', get_string('author_id', 'iassign'));                   // author_id          ='Author activity:'
    $mform->addElement('static', 'author_modified', get_string('author_id_modified', 'iassign')); // author_id_modified ='Last modified by:'

    // Adding the standard "name" field: the title of this activity
    //  Title [                       ]
    ...
    $selectElems = array($html_group => $arrayHTML, $java_group => $arrayJava);
    ...
    // Select the iLM associated to this activity/file
    ...
    // if (isset($this->_customdata['iassign_ilmid'])) $iassign_ilmid = $this->_customdata['iassign_ilmid'];
    if (isset($mform->_attributes['iassign_ilmid'])) { // if it is first acces, define 'iassign_ilmid'
      // ATTENTION, if the iLM is disabled, it is not selected under the JavaScript command bellow
      // $mform->_attributes = Array([id] => mform1 [iassign_ilmid] => 1 [autocomplete] => off [action] => http://localhost/dev_saw/mod/iassign/view.php [method] => post [accept-charset] => utf-8 [class] => mform ) 
      $iassign_ilmid = $mform->_attributes['iassign_ilmid'];
      // If the page has more then one 'mform' the line bellow could fail, it is better to get "mform" by its id="mform1"
      // $code_javascript .= ' <script>  document.forms[0].iassign_ilmid.value = ' . $iassign_ilmid . '</script>' . "\n";
      $code_javascript .= ' <script>  document.forms["mform1"].iassign_ilmid.value = ' . $iassign_ilmid . '</script>' . "\n";
      }
    else
    if ($result_recent && isset($result_recent->iassign_ilmid)) { // try the last iLM used...
      ...
      $code_javascript .= ' <script>  document.forms["mform1"].iassign_ilmid.value = ' . $result_recent->iassign_ilmid . '</script>' . "\n";
      }

    //TODO Adjusts to use API of MoodleForm
    ...
    $fileurl = "";
    $filename = "";
    if (!is_null($COURSE->iassign_file_id)) { // "$COURSE->iassign_file_id" must be filled in "locallib.php : function add_edit_iassign(): ... = $file_item->get_id()"
    ...
    //D echo "iassign_form.php: COURSE->id=" . $COURSE->id . ", COURSE->iassign_file_id=" . $COURSE->iassign_file_id . ", filename=" . $filename . "<br/>\n";// exit;


    // Form field: "Choose the file with the iLM activity"
    // Here appears the file name (content to the iLM)
    $html_div = '<div id="fitem_id_iassign_file_id" class="fitem required fitem_fgroup" style="padding: 35px; padding-left: 0;">' . chr(13);
    ...
    $html_div .= '<span><a><i class="icon fa fa-exclamation-circle text-danger fa-fw " title="' . get_string('requiredelement', 'form') . '" aria-label="' . get_string('requiredelement', 'form') . '" style="float: right; cursor: help; padding-right: 15px;"></i></a></span></div>' . chr(13);
    $html_div .= '<div class="felement fselect" id="error_message_file"><div class="file_iassign" id="file_border" style="display: inline;margin-left: 14px;border: 1px solid #cecfd1;padding: 8px;padding-right: 8px;border-radius: 4px;padding-right: 2px;">' . chr(13);
    $html_div .= '<i class="icon fa fa-file-text-o fa-fw" id="icon_doc" style="color: #8f8f8f;"></i><span id="iassign_file_link" style="color:#000000;"><a href="' . $fileurl . '" target="_blank" title="' . get_string('download_file', 'iassign') . $filename . '">' . $filename . '</a></span>' . chr(13);
    ...
    $mform->addElement('modgrade', 'grade', get_string('grade', 'iassign') . ':'); // 'grade'='Grade activity'
